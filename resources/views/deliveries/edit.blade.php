    ...
    <div class="container-fluid">
        <div class="row">
            <div class="col-12 text-center my-5">
                ...
                <h2 class="text-muted">Modifier la livraison {{ $delivery->id }}</h2>
            </div>
            <div class="col-xs-12 col-md-10 offset-md-1">
                <div class="card">
                    <div class="card-block">
                        <h4 class="text-center my-3">Modifier une livraison</h4>

                        <div class="col-xs-12 col-xl-8 offset-xl-2">
                            <form method="POST" action="{{ url("/deliveries/{$delivery->id}") }}" role="form">
                                {{ method_field('PUT') }}
                                {{ csrf_field() }}
                                ...
                                <div class="form-group{{ $errors->has('order_id') ? ' has-error' : '' }}">
                                    ...
                                    <select name="order_id" id="order_id" class="custom-select form-control" required>
                                        ...
                                        @forelse ($orders as $order)
                                            <option value="{{ $order->id }}" {{ $delivery->order_id == $order->id ? 'selected' : '' }}>{{ $order->reference }}</option>
                                        @empty
                                ...
                                <div class="form-group{{ $errors->has('contact_id') ? ' has-error' : '' }}">
                                    ...
                                    <select name="contact_id" id="contact_id" class="custom-select form-control" required>
                                        ...
                                        @forelse ($contacts as $contact)
                                            <option value="{{ $contact->id }}" {{ $delivery->contact_id == $contact->id ? 'selected' : '' }}>{{ "{$contact->company->name} / {$contact->name}" }}</option>
                                        @empty
                                ...
                                <div class="form-group{{ $errors->has('internal_comment') ? ' has-error' : '' }}">
                                    ...
                                    <input type="text" id="internal_comment" name="internal_comment" class="form-control"
                                           value="{{ $delivery->internal_comment }}" placeholder="Commentaire">
