    ...
    <div class="container-fluid">
        <div class="row">
            <div class="col-12 text-center my-5">
                ...
                <h2 class="text-muted">Ajouter une nouvelle commande</h2>
            </div>
            <div class="col-xs-12 col-md-10 offset-md-1">
                <div class="card">
                    <div class="card-block">
                        <h4 class="text-center my-3">Ajouter une commande</h4>

                        <div class="col-xs-12 col-xl-8 offset-xl-2">
                            <form method="POST" action="{{ url('/orders') }}" role="form">
                                {{ csrf_field() }}
                                ...
                                <div class="form-group{{ $errors->has('status') ? ' has-error' : '' }}">
                                    ...
                                    <select name="status" id="status" class="custom-select form-control" required>
                                        ...
                                        <option value="nok" {{ old('status') == 'nok' ? 'selected' : '' }}>Nok</option>
                                    </select>
                                ...
                                <div class="form-group{{ $errors->has('business_id') ? ' has-error' : '' }}">
                                    ...
                                    <select name="business_id" id="business_id" class="custom-select form-control" required>
                                        ...
                                        @forelse ($businesses as $business)
                                            <option value="{{ $business->id }}" {{ old('business_id') == $business->id ? 'selected' : '' }}>{{ "{$business->company->name} / {$business->name}" }}</option>
                                        @empty
