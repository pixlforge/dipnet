<template>
    <div>
        ...
        <div class="col-12 col-lg-2">
            ...
            <!--Reference-->
            <h5 class="mb-0">
                <a :href="`/orders/${order.reference}/create`">{{ order.reference }}</a>
            </h5>
        </div>
        ...
        <div class="col-12 col-lg-2">
            ...
            <!--Status-->
            <span class="badge badge-custom" :class="statusClass" v-text="order.status"></span>
        </div>

        <div class="col-12 col-lg-3">

            <!--Business-->
            <div class="card-content" v-if="order.business">
                <span class="card-label">Affaire:</span>
                <span v-text="order.business.name"></span>
            </div>

            <!--Contact-->
            <div class="card-content" v-if="order.contact">
...
<script>
    ...
    export default {
        ...
        computed: {
            statusClass() {
                if (this.order.status === 'incomplete') return 'badge-warning';
                if (this.order.status === 'completed') return 'badge-success';
            }
        },
        methods: {
            getDate(date) {
